...
def get_message(hub, msg):
    ...
    with NamedTemporaryFile(prefix="devpi-testfile-", suffix=".txt") as tf:
        tf.write(
            textwrap.dedent(
                """\n
        ...
        if result != 0:
            hub.fatal("Error ({}) calling editor {}".format(result, editor))
        tf.seek(0)
...
@contextmanager
def devpi_testfile_review_lock(hub):
    if not os.path.exists(devpi_testfile_data_dir):
        os.mkdir(devpi_testfile_data_dir)
    lock_fn = os.path.join(devpi_testfile_data_dir, "reviews.lock")
    try:
        with open(lock_fn, "x"):
            yield
        ...
        hub.fatal(
            ...
            "This can happen if a previous devpi-testfile command crashed. "
            "If you are sure there is no other devpi-testfile command still running, "
...
@contextmanager
def devpi_testfile_review_data(hub):
    with devpi_testfile_review_lock(hub):
        fn = os.path.join(devpi_testfile_data_dir, "reviews.json")
...
def require_pr_index(hub, name):
    ...
    if result.reason != "OK":
        hub.fatal("Couldn't access pr index '{}': {}".format(name, result.reason))
...
def new_pr(hub, args):
    ...
    hub.http_api(
        "put",
        url,
        dict(type="pr", bases=target, states=["new"], messages=["New pull request"]),
    )
    for req in reqs:
        hub.http_api(
            "push",
...
def abort_pr_review(hub, args):
    ...
    with devpi_testfile_review_data(hub) as review_data:
...
def approve_pr_arguments(parser):
    ...
    parser.add_argument(
        "-m", "--message", action="store", help="Message to add on submit."
    )
    parser.add_argument(
        "-k",
        "--keep-index",
...
def approve_pr(hub, args):
    ...
    hub.http_api(
        "patch",
        indexinfos.url,
        ["states+=approved", "messages+=%s" % message],
...
def list_prs_arguments(parser):
    ...
    parser.add_argument(
        "-a", "--all-states", action="store_true", help="Output normally hidden states."
    )
    parser.add_argument(
        "-m",
        "--messages",
...
def merge_pr_data(data1, data2):
    ...
    for state in states:
        ...
        for user in users:
            user_data1 = {
                tuple((k, tuple(v) if isinstance(v, list) else v) for k, v in x.items())
            ...
            state_data[user] = list(dict(x) for x in user_data1.union(user_data2))
    return result
...
def get_prs(users_prs):
    ...
    for user, prs in users_prs.items():
        for pr in prs:
            result.append(dict(pr, name="{}/{}".format(user, pr["name"])))
...
def create_pr_list_output(users_prs, review_data, include_messages):
    ...
    for pr in prs:
        if pr["name"] in review_data:
            ...
            active = ""
        out.append(fmt.format(pr["name"], pr["base"], pr["last_serial"], active))
...
def list_prs(hub, args):
    ...
    hub.info("current devpi index: {} ({})".format(current.index, login_status))
    if user:
    ...
    for state in sorted(pr_data):
        if state in hidden_states:
            continue
        with devpi_testfile_review_data(hub) as review_data:
            out = create_pr_list_output(pr_data[state], review_data, args.messages)
...
def reject_pr_arguments(parser):
    ...
    parser.add_argument(
        "-m", "--message", action="store", help="Message to add on reject."
    )
...
def reject_pr(hub, args):
    ...
    message = get_message(hub, args.message)
    hub.http_api(
        "patch", indexinfos.url, ["states+=rejected", "messages+=%s" % message]
    )
...
def review_pr_arguments(parser):
    ...
    parser.add_argument(
        "-u", "--update", action="store_true", help="Update the serial of the review."
    )
...
def review_pr(hub, args):
    ...
    with devpi_testfile_review_data(hub) as review_data:
        if indexinfos.indexname in review_data:
            if args.update:
                hub.info(
                    "Updated review of '{}' to serial {}".format(
                        indexinfos.indexname, last_serial
                    )
                )
            else:
                hub.warn(
                    "Already reviewing '{}' at serial {}".format(
            ...
            hub.info(
                "Started review of '{}' at serial {}".format(
                    indexinfos.indexname, last_serial
                )
            )
...
def submit_pr(hub, args):
    ...
    indexinfos = require_pr_index(hub, name)
    message = get_message(hub, args.message)
    hub.http_api("patch", indexinfos.url, ["states+=pending", "messages+=%s" % message])
...
def cancel_pr_arguments(parser):
    ...
    parser.add_argument(
        "-m", "--message", action="store", help="Message to add on cancel."
    )
...
def cancel_pr(hub, args):
    ...
    indexinfos = require_pr_index(hub, name)
    message = get_message(hub, args.message)
    hub.http_api("patch", indexinfos.url, ["states+=new", "messages+=%s" % message])
